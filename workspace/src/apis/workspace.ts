 ...
 * 查询项目列表
 */
export const workspace_list = async (params: any = {}) => {
  let whereClause = "";
  ...
  const queryData = {
    path: "workspace",
    sql: `SELECT * FROM workspace ${whereClause}`,
  };
 ...
 * 根据 ID 获取项目
 */
export const workspace_get = async (id: string | number) => {
  const queryData = {
    path: "workspce",
    sql: `SELECT * FROM workspace WHERE id = ${
 ...
 * 更新项目
 */
export const workspace_update = async (data: any) => {
  ...
  const updateData = {
    path: "workspce",
    sql: `UPDATE workspace SET ${setClause} WHERE id = ${
      typeof id === "string" ? `'${id}'` : id
    }`,
  };
 ...
 * 删除项目
 */
export const workspace_delete = async (data: { id: string | number }) => {
  if (!data.id) {
