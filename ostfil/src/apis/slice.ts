 ...
 * 查询项目列表
 */
export const ostfil_list = async (params: any = {}) => {
  let whereClause = "";
  ...
  const queryData = {
    path: "ostfil_slice",
    sql: `SELECT * FROM ostfil_slice ${whereClause}`,
  };
 ...
 * 根据 ID 获取项目
 */
export const ostfil_get = async (id: string | number) => {
  const queryData = {
    path: "ostfil_slice",
    sql: `SELECT * FROM ostfil_slice WHERE id = ${
 ...
 * 更新项目
 */
export const ostfil_update = async (data: any) => {
  ...
  const updateData = {
    path: "ostfil_slice",
    sql: `UPDATE ostfil_slice SET ${setClause} WHERE id = ${
      typeof id === "string" ? `'${id}'` : id
    }`,
  };
...
export const project_delete = async (data: { id: string | number }) => {
  ...
  const deleteData = {
    path: "ostfil_slice",
    sql: `DELETE FROM ostfil_slice  WHERE id = ${
