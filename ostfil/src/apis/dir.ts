 ...
 * 查询项目列表
 */
export const dirs_list = async (params: any = {}) => {
  let whereClause = "";
  ...
  const queryData = {
    path: "dirs",
    sql: `SELECT * FROM dirs${whereClause}`,
  };
 ...
 * 根据 ID 获取项目
 */
export const dirs_get = async (id: string | number) => {
  const queryData = {
    path: "dirs",
    sql: `SELECT * FROM dirs WHERE id = ${
 ...
 * 更新项目
 */
export const dirs_update = async (data: any) => {
  ...
  const updateData = {
    path: "dirs",
    sql: `UPDATE dirs SET ${setClause} WHERE id = ${
      typeof id === "string" ? `'${id}'` : id
    }`,
  };
